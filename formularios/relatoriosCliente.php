<div  class="content-box-large ">
	...
	<script>
		$("form#relatorios").on('submit', function() {
			...
			$("#conteudo").load("formularios/relatoriosCliente.php",{
				tabela: $rola[0].value,
				pesq: $pesq
			});
	...
	$rBLL = new relatorioBLL();
		...
		else 
		{
			...
			if ($pesq == "")
			{
				$sql= "SELECT pedido.id,assistente.nome,pedido.local,pedido.data_hora,pedido.status FROM pedido JOIN assistente ON pedido.assistente_id = assistente.id AND cliente_id =".$_SESSION['user']->fk;
			...
			ECHO $rBLL->pesqClientePedido($sql);
